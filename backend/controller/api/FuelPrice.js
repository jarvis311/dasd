...
const getFualPriceByCity = async (req, res) => {
  ...
  try {
    ...
    if (await redisClient.get(`FuelPricebyCity_${city}_${state}_${today}`)) {
      ...
      let citypricefortoday = await FuelPrice.findOne({
        where: {
          state: state,
          city: city,
          date: today,
        },
      }); console.log("====>",citypricefortoday);
      if (citypricefortoday!==null) {
      ...
      if (citypricefortoday==null) {
        ...
        if (
          ...
          let citypriceForyesterday = await FuelPrice.findOne({
            where: {
              state: state,
              city: city,
              date: yesterday,
          ...
          if (citypriceForyesterday==null) {
            if (
              ...
              let old_cityprice = await FuelPrice.findOne({
                where: {
                  state: state,
                  city: city,
                  [Op.not]: {
