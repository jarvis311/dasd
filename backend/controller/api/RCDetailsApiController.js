...
export const RCDetailsApiController = async (req, res) => {
    ...
    try {
        ...
        if (!reg_number || !/^[a-zA-Z0-9]+$/.test(reg_number?.toUpperCase()) || reg_number == "" || reg_number?.length < 10) {
            const response = utils.encrypt(JSON.stringify({
                status: false,
                response_code: 400,
...
export async function rcDetails (data){
    try {
        let { type, version_code, reg_number, is_skip_db, user_id } = data;
        let logResponse = "";
        const key = 'V@$undh@r@50599#';
        const versioncode = version_code || '';
        const appType = type || '';
        ...
        // CHECK IF STATE IS PRESENT IN DATABASE  
        if (checkIsState) {
            const query = `SELECT * FROM ${newRegFirstTwoChar} WHERE reg_no = :reg_number`;
            const [results, metadata] = await db.Vehicle.query(query, {
                replacements: { reg_number: reg_number },
                type: Sequelize.QueryTypes.SELECT
            });
...
export const  rc_fail_data_remove = async(req,res) =>{
   try {
    ...
    const query1 = `DELETE FROM fail_data WHERE reg_no='${reg_number}'`
     const [results, metadata] = await db.Vehicle.query(query1);
        console.log('data is deleted ');
        let resp = utils.encrypt({
            status:true,
            response_code:200,
            response_message:"Data Delete Successfully"
        },rto)
        return res.send(resp)
...
export const  rc_fail_data_store =async(req,res)=>{
try {
 console.log(req.body);
 let rto = req.body.hasOwnProperty('rto');
...
export const rc_remind_me_remove = async(req,res)=>{
    try {
        console.log('hiiiiiii');
        console.log(req.body);
 let rto = req.body.hasOwnProperty('rto');

        let {user_id,reg_number,doc_type} = req.body;
        if (!reg_number||reg_number=="") {
            let resp = utils.encrypt({
                status:false,
                response_code : 400,
            ...
            return res.send(resp)
        }else if (!doc_type || doc_type=="") {
            let resp = utils.encrypt({
                status:false,
                response_code : 400,
                response_message : "doc_type Field Required"
            },rto)
            return res.send(resp)
        }
        let deletequery = `DELETE FROM rc_reminder WHERE user_id=${user_id} AND reg_number='${reg_number}' AND doc_type=${doc_type}`
        let deletereminder = await db.Vehicle.query(deletequery)
         let resp = utils.encrypt({
            status:true,
            response_code :200,
            response_message:"Reminder Remove Successfully"
         },rto)
     return res.send(resp)
