...
const affilation = async (req,res) =>{
      ...
      for (const program of affilation_program) {
        ...
        if (affilateData) {
          ...
          if (type !== 'ios' && input.version_code >= 10.06 && (place === 'home_bike' || place === 'home_car')) {
            ...
            isNeedToShow = await check_boolean(program.is_need_to_show);
          }
  ...
  if (home_slider_id) {
    const serviceProgram = await Affilation.findAll({
      ...
      //order: [['id', 'ASC'],[Sequelize.fn('RAND')],[ Sequelize.col('get_affiliation_data.position', 'ASC') ]],
      order: [[ Sequelize.col('get_affiliation_data.position', 'ASC') ],[ Sequelize.col('get_affiliation_data.id',[Sequelize.fn('RAND')]) ],['id', 'ASC']],
      attributes: ['is_need_to_show', 'id', 'service_category_id'],
    });
    ...
    if (serviceProgram.length !== 0) {
      for (const program of serviceProgram) {
        ...
        for (const datas of affilateData) {
          let  data = await AffilationData.findAll({
            where: {
              affiliate_id:program.id,
          ...
          var service_provider_id = false;
          var affiliation_services_id = false;
          if (data &&  data.length) {
              for(const singleData of data)
              {
                ...
                for (const singleAffilation of AffiationProviderArray) {
                  if (
                    singleAffilation.sp_id === singleData.service_provider_id &&
                    singleAffilation.as_id === singleData.affiliation_services_id
                  ) {
                    
                    service_provider_id = singleData ? affiationPriorityServiceProviderArray.includes(singleData.service_provider_id) : false;
                    affiliation_services_id = singleData ? affiationPriorityServiceIdArray.includes(singleData.affiliation_services_id) : false;
                    data = singleData
                    break ;
                  }
            ...
            if (affiliation_services_id  && service_provider_id && data ) {
