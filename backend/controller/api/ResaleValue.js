...
import { rcDetails } from "./RCDetailsApiController.js"
...
const getUserVehicleSearch = async(req,res)  => {
    try {
        ...
        // Find by Rc Details via reg_number
        let response = await rcDetails(reg_no)
        if(response && response.status){
            let vehicleData = response.data
            let category,makerName,maker
            if('M-CYCLE/SCOOTER (2WN),M-Cycle/Scooter(2WN),Moped(2WN)'.toLowerCase().indexOf(vehicleData.vh_class.toLowerCase()) !== -1 ) {
            // if(['M-CYCLE/SCOOTER (2WN)'.toLowerCase(), 'M-Cycle/Scooter(2WN)'.toLowerCase(), 'Moped(2WN)'.toLowerCase()].includes(vehicleData.vh_class.toLowerCase())) {
                ...
                for (const val of maker) {
                    ...
                    if(vehicleCompany.length){
                        vehicleCompany = vehicleCompany.map(c => c.id);
                    }
            ...
            // }else if (['Motor Car(LMV)'.toLowerCase()].includes(vehicleData.vh_class.toLowerCase())){
            }else if('Motor Car(LMV)'.toLowerCase().indexOf(vehicleData.vh_class.toLowerCase())!== -1 ){
                category = 1;
            } else {
                category = 0;
            }
                ...
                if(!vehicleCompany){
                    ...
                    if(vehicleModel){
                        ...
                        response = await ResaleVehicleModel.findAll({
                            ...
                            attributes:['id',['id','ModalID'],['name','ModelName'],'resale_vehicle_company_id'],
                        })
                    }
                    ...
                    return res.send(utils.encrypt(finalResponse,rto))
                }
            }else{
                const response = {
                    status : false,
