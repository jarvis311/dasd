...
const driving_school_state = async (req, res) => {
  try {
    ...
    if (await redisClient.get("driving_school_state")) {
      ...
      // await  redisClient.del("driving_school_state")
      if (data) {
...
const add_driving_school_details = async (req, res) => {
  try {
    ...
    if (!city_id || city_id === undefined) {
      ...
      return res.send(resp)
    }else if(!driving_school_address || driving_school_address === 0){
      let resp = utils.encrypt({
        status: false,
        response_code: 400,
        msg: "Please provide address of school field!!",
      },rto)
      return res.send(resp)
    }
  ...
  try {
    ...
    if (!state_id || state_id === undefined) {
      let resp = utils.encrypt({
        status: false,
        response_code: 400,
        msg: "Please provide the state_id",
      },rto)
      return res.send(resp)
    }
  ...
  try {
    ...
    if (!city_id || city_id === undefined) {
      let resp = utils.encrypt({
        status: false,
        response_code: 400,
        msg: "Please provide the city_id",
      },rto)
      return res.send(resp)
    }
