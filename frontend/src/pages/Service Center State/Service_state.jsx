...
const Service_state = () => {
  ...
  const GetData = async () => {
    const result = await API.post("/get_service_center_state", { limit: size, page: current }, { headers: { Authorization: `Bearer ${token}` } })
    ...
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          ...
          // form.append('id', id)
          await API.post(`/delete_service_center_state/${id}`, {}, { headers: { Authorization: `Bearer ${token}` } });
          GetData();
        } else {
          count = 10
          clearInterval(swalCountdownInterval)
  ...
  const Seaching = async (e, name) => {
    ...
    let result
    if (e.target.value == "") {
      result = await API.post("/get_service_center_state", { limit: size, page: current }, { headers: { Authorization: `Bearer ${token}` } });
      setData(result.data.data);
      setTotalPageCount(result?.data?.totalcount)
    }
    else {
      result = await API.post("/search_driving_school", Form, { headers: { Authorization: `Bearer ${token}` } });
      setData(result.data.data);
      setTotalPageCount(result?.data?.totalcount);
    }
  ...
  const Togglechange = async (id, e, name) => {
    ...
    const result = await API.post("/Servicestatuschange", Form, { headers: { Authorization: `Bearer ${token}` } });
    if (result) {
                  ...
                  Data?.map((val, i) => {
                    return (
                      <tr>
                        ...
                        <td className='text-center'>
                          <Link to={`/view/service_state/${val.id}`}>
                ...
                loading == false && totalPageCount === 0 ? <tr>
                  <td colSpan="100%" className="p-0">
                    <div className='no-found'>
                      ...
                      <p>No Found Service Center State</p>
                    </div>
                  </td>
                </tr> : ""
              }
