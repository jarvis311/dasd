...
const View_service_state = () => {
  ...
  const Getview = async (Eid) => {
    const result = await API.post(`/edit_driving_school_state/${Eid !== "" ? Eid : id.id}`, {}, { headers: { Authorization: `Bearer ${token}` } });
    SetData({
  ...
  return (
    <Layout sidebar={true}>
      <div className="page-heading">
        <h3><Link to="/service_state" className='btn btn-primary btn-icon-lg me-3'><i className='bx bxs-left-arrow-alt'></i></Link>View Service Center State</h3>
        <div className="page-heading-right">
          <SelectPicker data={DropDropValue} value={id.id} defaultValue={id.id} cleanable={false} className="wv-200 my-1 ms-3" onChange={(e) => selectpickerData(e)} placeholder="Select State Name" placement="bottomEnd" />
          <Link to={`/edit/service_state/${id.id}`}>
