...
const Edit_News_headline = () => {
  ...
  const Cate_hendler = async (e) => {
    ...
    brand_onload(Array.isArray(e) ? e.map(x => x.value) : [])
  ...
  return { value: val._id, label: val.name }
    ...
    const Get_view = async () => {
        ...
        if(result.data.status===true){
            ...
            brand_onload(Cate_array)
            
            var Brand_array = []
            result.data.Data.map((resVal) => {
                resVal.brand_id.map((val, i) => {
                    Brand_Data.push({ value: val._id, label: val.name })
                    Brand_array.push(val._id)
                })
            })
            setBrand_data(Brand_array)
    ...
    const Submit = async () => {
        if (NewData.title == "" || NewData.description == "" || NewData.news_url == "" || NewData.date == "" || N_category.length===0 || Category.length===0 || Brand_data.length===0 || suggetion.length===0) {
                                        ...
                                        <InputGroup.Text>
                                            <Fancybox>
                                                <a data-fancybox="gallery" href={NewData.image}>
                                                    <img src={NewData.image} className="hv-30 rounded-3" alt="" />
                                                </a>
                                            </Fancybox>
                                        </InputGroup.Text>
                                    </InputGroup>
                                        ...
                                        <InputGroup.Text>
                                            <Fancybox>
                                                <a data-fancybox="gallery" href={NewData.websiteimage}>
                                                    <img src={NewData.websiteimage} className="hv-30 rounded-3" alt="" />
                                                </a>
                                            </Fancybox>
                                        </InputGroup.Text>
                                    </InputGroup>
