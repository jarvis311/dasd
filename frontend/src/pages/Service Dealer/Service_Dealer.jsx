...
const Service_Dealer = () => {
  ...
  const GetData = async () => {
    const result = await API.post("/get_all_service_dealer", { limit: size, page: current }, { headers: { Authorization: `Bearer ${token}` } })
  ...
  const brand_name_dropdown = async (e) => {
    ...
    const Result = await API.post("/get_service_center_brand_option", Form, { headers: { Authorization: `Bearer ${token}` } })
    brandnameArray = []
    ...
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          const form = new FormData()
          form.append('id', id)
          await API.post(`/delete_service_dealer/${id}`, form, { headers: { Authorization: `Bearer ${token}` } });
          GetData();
        } else {
          count = 10
          clearInterval(swalCountdownInterval)
                  ...
                  getData1(current, size).map((val, i) => {
                    ...
                    return (
                      <tr>
                        ...
                        {/* <td>1</td> */}
                        <td className='text-center'>
                          <Link to={`/view/service_Dealer/${val.id}`}>
