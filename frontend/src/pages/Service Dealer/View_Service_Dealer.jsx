...
const View_Service_Dealer = () => {
  ...
  const Dropdown_Name = async () => {
    const Result = await API.post("/get_all_service_dealer", {}, { headers: { Authorization: `Bearer ${token}` } })
    DropDownArr = []
    reloadId = []
    Result.data.Data.map((val, i) => {
      DropDownArr.push({ label: val?.city_id.name, value: val.id })
  ...
  const Getview = async (Eid) => {
    const result = await API.post(`/get_service_dealer/${Eid !== "" ? Eid : id.id}`, {}, { headers: { Authorization: `Bearer ${token}` } });
    SetData({
    ...
    swalWithBootstrapButtons
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          ...
          const result = await API.post(`/delete_service_dealer/${id}`, {}, { headers: { Authorization: `Bearer ${token}` } });
          if (reloadId.length === 0) {
            ...
            // window.location.replace(`${process.env.REACT_APP_BASE_URL}view/service_Dealer/${deloadId}`)
          }
        } else {
          count = 10
          clearInterval(swalCountdownInterval)
    ...
    <Layout sidebar={true}>
      <div className="page-heading">
        ...
        <div className="page-heading-right">
          <SelectPicker data={DropDropValue} defaultValue={id.id} cleanable={false} className="wv-200 my-1 ms-3" onChange={(e) => selectpickerData(e)} placeholder="Select City Name" placement="bottomEnd" />
          <Link to={`/Edit/service_Dealer/${id.id}`}>
